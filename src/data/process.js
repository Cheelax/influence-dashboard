...
const getListByProcessorType = (processorType) =>
  Object.values(TYPES).filter((type) => type.processorType === processorType);
  ...
  return Object.keys(process.outputs).map((outputId) => {
    if (Number(actualTarget) === Number(outputId)) {
      ...
      return {
        id: Number(outputId),
        amount: Math.floor(
          process.outputs[outputId] * recipes * secondaryAdjust
        ),
      };
    }
  });
